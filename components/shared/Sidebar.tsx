...
export const Sidebar: React.FC = () => {
  ...
  const navItems = [
    ...
    { label: 'Compliance & NOC', href: '/dashboard/compliance', icon: <FileCheck className="h-5 w-5 shrink-0" /> },
    ...
    { label: 'Corporate Billing', href: '/dashboard/corporate-billing', icon: <Banknote className="h-5 w-5 shrink-0" /> },
  ...
  const RenderContent = () => (
    <div className="h-full flex flex-col justify-between bg-white border-r border-slate-100 text-slate-600">
      ...
      <div className="flex flex-col">
        <div className="h-16 flex items-center justify-between px-6 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-2.5 overflow-hidden">
            ...
            {!isCollapsed && (
              <motion.span
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="text-base font-bold tracking-tight text-slate-900 font-plus-jakarta shrink-0"
              >
          ...
          <button
            ...
            className="hidden lg:flex items-center justify-center p-1 rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-500 hover:text-slate-900 cursor-pointer outline-none transition-colors border border-slate-200"
          >
              ...
              <Link
                ...
                className={`
                  ...
                  ${isActive
                    ...
                    : 'hover:bg-orange-50/50 text-slate-500 hover:text-primary'
                  }
                `}
              >
                {item.icon}
        ...
        {isMobileOpen && (
          <div className="fixed inset-0 z-50 lg:hidden flex">
            ...
            >
              <button
                onClick={() => setMobileOpen(false)}
                className="absolute top-4 right-[-45px] p-2 bg-white border border-slate-100 text-slate-700 rounded-xl shadow-lg cursor-pointer animate-fadeIn"
              >
