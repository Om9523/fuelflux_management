...
/**
 * FILE: app/dashboard/inventory/components/TanksTab.tsx
 * Existing tanks UI — extracted from original inventory page, no changes.
 */
...
export default function TanksTab() {
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            {/* Sub-header */}
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white border border-slate-100 rounded-2xl p-5 shadow-xs gap-4 text-left">
                <div>
                    <h2 className="text-sm font-extrabold text-text-primary">Active Storage Tanks</h2>
                    <p className="text-xs text-text-secondary mt-0.5">
                        Real-time digital fuel levels, wet-stock adjustments and sensor audits
                    </p>
                </div>
                <button
                    onClick={handleReconcile}
                    className="inline-flex items-center gap-2 rounded-xl bg-primary hover:bg-primary/90 px-4 py-2.5 text-xs font-bold text-white shadow-md shadow-primary/20 transition-all cursor-pointer"
                >
                    Reconcile Wet-Stock
                </button>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-6 text-left">
                {tanks.map((t) => {
                    ...
                    return (
                        <div
                            key={t.id}
                            className={`bg-white border rounded-3xl p-6 shadow-sm flex flex-col gap-6 relative transition-all duration-300
                        ...
                        <tbody className="divide-y divide-slate-100 font-medium text-text-primary">
                            {deliveries.map((d) => (
                                <tr key={d.id} className="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="p-4 font-bold font-mono">{d.volume}</td>
                                    <td className="p-4">
                                        <span className="flex items-center gap-1.5">
                                            <Calendar className="h-3.5 w-3.5 text-slate-400" />{d.date}
                                        </span>
                                    </td>
