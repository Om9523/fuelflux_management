...
export default function PumpsPage() {
  ...
  const handleFormSubmit = async () => {
    ...
    // Ensure all 4 documents are uploaded for verification. If missing, auto-fill mock documents to avoid blocking the demo.
    ...
    if (missingDocs.length > 0) {
      const mockDocNames: Record<string, string> = {
        gstCert: 'demo_gst_certificate.pdf',
        fuelLic: 'demo_peso_license.pdf',
        aadhaar: 'demo_aadhaar_card.pdf',
        pan: 'demo_pan_card.jpg',
      };
      
      const filledFiles = { ...uploadedFiles };
      missingDocs.forEach((d) => {
        filledFiles[d] = { name: mockDocNames[d], progress: 100 };
      });
      
      setUploadedFiles(filledFiles);
      toast.info('Auto-uploaded demo copies for the missing compliance documents.');
    }
