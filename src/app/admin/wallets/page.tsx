      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex justify-between items-center">
          <div className="space-y-1">
            ...
            <span className="text-2xl font-extrabold text-slate-900 font-mono">₹{totalCreditLimit.toLocaleString('en-IN')}</span>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex justify-between items-center">
          <div className="space-y-1">
            ...
            <span className="text-2xl font-extrabold text-slate-900 font-mono">₹{totalCreditUsed.toLocaleString('en-IN')}</span>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex justify-between items-center">
          <div className="space-y-1">
            ...
            <span className="text-2xl font-extrabold text-slate-900 font-mono">
              ₹{walletTopups.reduce((sum, p) => sum + p.amount, 0).toLocaleString('en-IN')}
            </span>
          </div>
                  ...
                  <div key={partner.id} className="space-y-2 border border-slate-100 rounded-xl p-4 bg-slate-50/50">
                    <div className="flex justify-between items-center text-xs font-semibold">
                      ...
                      <span className="font-mono text-slate-600">
                        ₹{partner.creditUsed.toLocaleString('en-IN')} / ₹{partner.creditLimit.toLocaleString('en-IN')}
                      </span>
                    </div>
                ...
                <div key={topup.id} className="flex justify-between items-start text-xs border border-slate-100 p-3 rounded-xl bg-slate-50/20">
                  ...
                  <div className="text-right space-y-1">
                    <span className="font-mono font-bold text-emerald-600 block">
                      +₹{topup.amount.toLocaleString('en-IN')}
                    </span>
