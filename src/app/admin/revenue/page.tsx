      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex justify-between items-center">
          <div className="space-y-1">
            ...
            <span className="text-2xl font-extrabold text-slate-900 font-mono">₹{mrr.toLocaleString('en-IN')}</span>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex justify-between items-center">
          <div className="space-y-1">
            ...
            <span className="text-2xl font-extrabold text-slate-900 font-mono">₹{arr.toLocaleString('en-IN')}</span>
          </div>
          ...
          <div className="h-64 w-full font-semibold text-slate-400">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={revenueGrowthHistory} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                ...
                <Tooltip formatter={(val: any) => [typeof val === 'number' ? `₹${val.toLocaleString('en-IN')}` : val, 'Revenue']} contentStyle={{ backgroundColor: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', fontSize: '11px', fontWeight: '600' }} />
