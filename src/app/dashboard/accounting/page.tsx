...
// Mock data structured by pump ID for reactive updates
const ACCOUNTING_SEEDS: Record<string, {
  revenue: number;
  expenses: number;
  profitMargin: number;
  gstPayable: number;
  plItems: Array<{ category: string; amount: number; percentage: number; type: 'income' | 'expense' }>;
  expensesLog: Array<{ id: string; desc: string; category: string; date: string; amount: number; paymentMode: string; status: 'cleared' | 'pending' }>;
  gstLog: Array<{ period: string; cgst: number; sgst: number; status: 'filed' | 'due' | 'overdue' }>;
}> = {
  pump_1: {
    revenue: 14820900,
    expenses: 12140500,
    ...
    gstPayable: 195600,
    plItems: [
      { category: 'Fuel Product Sales (Retail)', amount: 13200900, percentage: 89, type: 'income' },
      { category: 'Commercial Credit Vouchers', amount: 1420000, percentage: 9, type: 'income' },
      { category: 'Lube Shop & Convenio Inflow', amount: 200000, percentage: 2, type: 'income' },
      { category: 'Fuel Tanker Consignments', amount: 9850000, percentage: 81, type: 'expense' },
      { category: 'Forecourt Attendants Wages', amount: 750000, percentage: 6, type: 'expense' },
      { category: 'Operational Electricity (Heavy Pumps)', amount: 310500, percentage: 2.5, type: 'expense' },
      { category: 'PESO & Gas Cylinder Hydrotesting Licenses', amount: 150000, percentage: 1.2, type: 'expense' },
      { category: 'Insurance & Fleet Maintenance', amount: 1080000, percentage: 8.8, type: 'expense' },
    ],
    expensesLog: [
      { id: 'EXP-801', desc: 'IndianOil HP-Speed Tanker Consignment', category: 'Fuel Purchases', date: '2026-05-24', amount: 2450000, paymentMode: 'RTGS Transfer', status: 'cleared' },
      { id: 'EXP-802', desc: 'Monthly Forecourt Shift Compensation', category: 'Salaries', date: '2026-05-01', amount: 750000, paymentMode: 'Direct Bank Deposit', status: 'cleared' },
      { id: 'EXP-803', desc: 'Electricity Board Commericial Draw', category: 'Utilities', date: '2026-05-15', amount: 310500, paymentMode: 'Auto-Debit ECS', status: 'cleared' },
      { id: 'EXP-804', desc: 'Sree Balaji Hydrotesting Compliance Charges', category: 'Compliance', date: '2026-05-18', amount: 150000, paymentMode: 'Net Banking', status: 'cleared' },
      { id: 'EXP-805', desc: 'Forecourt Roof LED Signage Retrofitting', category: 'Maintenance', date: '2026-05-26', amount: 80000, paymentMode: 'Corporate Card', status: 'pending' },
    ],
    gstLog: [
      { period: 'April 2026', cgst: 97800, sgst: 97800, status: 'filed' },
      { period: 'May 2026 (Provisional)', cgst: 97800, sgst: 97800, status: 'due' },
      { period: 'March 2026', cgst: 105200, sgst: 105200, status: 'filed' },
    ]
  },
  pump_2: {
    revenue: 9840200,
    ...
    gstPayable: 112000,
    plItems: [
      { category: 'Fuel Product Sales (Retail)', amount: 7200200, percentage: 73, type: 'income' },
      { category: 'Electric EV Charging Bays', amount: 1640000, percentage: 17, type: 'income' },
      { category: 'Convenience Mart & Premium Cafe', amount: 1000000, percentage: 10, type: 'income' },
      { category: 'Fuel Tanker Consignments', amount: 5600000, percentage: 73.5, type: 'expense' },
      { category: 'Staff Compensation', amount: 620800, percentage: 8.1, type: 'expense' },
      { category: 'EV Charging Grid Electricity Draw', amount: 900000, percentage: 11.8, type: 'expense' },
      { category: 'Operations & Maintenance', amount: 500000, percentage: 6.6, type: 'expense' },
    ],
    expensesLog: [
      { id: 'EXP-901', desc: 'Grid EV Fast Charging Infrastructure Draw', category: 'Utilities', date: '2026-05-22', amount: 450000, paymentMode: 'Auto-Debit ECS', status: 'cleared' },
      { id: 'EXP-902', desc: 'HPCL Premium Petrol Delivery', category: 'Fuel Purchases', date: '2026-05-20', amount: 1800000, paymentMode: 'RTGS Transfer', status: 'cleared' },
      { id: 'EXP-903', desc: 'Barista Espresso Machine Maintenance', category: 'Maintenance', date: '2026-05-12', amount: 35000, paymentMode: 'Corporate Card', status: 'cleared' },
      { id: 'EXP-904', desc: 'Gachibowli Station Salaries', category: 'Salaries', date: '2026-05-01', amount: 620800, paymentMode: 'Direct Bank Deposit', status: 'cleared' },
      { id: 'EXP-905', desc: 'Premium Cafe Bakery Sourcing Consignment', category: 'Inventory', date: '2026-05-27', amount: 48000, paymentMode: 'UPI Gateway', status: 'pending' },
    ],
    gstLog: [
      { period: 'April 2026', cgst: 56000, sgst: 56000, status: 'filed' },
      { period: 'May 2026 (Provisional)', cgst: 56000, sgst: 56000, status: 'due' },
      { period: 'March 2026', cgst: 64200, sgst: 64200, status: 'filed' },
    ]
  },
  default: {
    revenue: 0,
...
export default function AccountingPage() {
  ...
  // Load state based on active pump
  const currentPumpId = selectedPump?.id || 'pump_1';
  const pumpData = ACCOUNTING_SEEDS[currentPumpId] || ACCOUNTING_SEEDS.pump_1;

  const [expenses, setExpenses] = useState(pumpData.expensesLog);

  // Sync state whenever active pump changes
  useEffect(() => {
    setExpenses(pumpData.expensesLog);
  }, [currentPumpId, pumpData.expensesLog]);
  ...
  const handleAddExpense = (e: React.FormEvent) => {
    ...
    const newExp = {
      id: 'EXP-' + Math.floor(810 + Math.random() * 90),
    ...
    toast.success(`Expense Registered: ${expenseData.desc} (₹${numAmt.toLocaleString('en-IN')})`);
  ...
  const totalRevenue = pumpData.revenue;
  // Calculate expenses dynamically to reflect manually added ones
  const activeExpensesSum = expenses.reduce((sum, item) => sum + item.amount, 0);
  const totalExpenses = currentPumpId === 'pump_3' ? 0 : (activeExpensesSum + (pumpData.expenses - pumpData.expensesLog.reduce((s, i) => s + i.amount, 0)));
  const netProfit = Math.max(0, totalRevenue - totalExpenses);
  const profitMarginPercent = totalRevenue > 0 ? parseFloat(((netProfit / totalRevenue) * 100).toFixed(1)) : 0;
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex flex-col bg-slate-50/50 border border-slate-100 rounded-2xl p-4 divide-y divide-slate-100">
                        {pumpData.plItems.filter(i => i.type === 'income').map((item, idx) => (
                    ...
                    <div className="flex flex-col gap-2 mt-2">
                      ...
                      <div className="flex flex-col bg-slate-50/50 border border-slate-100 rounded-2xl p-4 divide-y divide-slate-100">
                        {pumpData.plItems.filter(i => i.type === 'expense').map((item, idx) => (
                          <div key={idx} className="flex justify-between py-2.5 text-xs first:pt-0 last:pb-0 font-medium">
                            <span className="text-slate-600 flex items-center gap-1.5">
