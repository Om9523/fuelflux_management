        ...
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
            <div className="p-3 bg-slate-50 border border-slate-100 rounded-xl">
              ...
              <span className="text-sm font-bold text-slate-800 mt-1 block font-mono">₹{latestSalary.monthlySalary.toLocaleString('en-IN')}</span>
            </div>
            <div className="p-3 bg-slate-50 border border-slate-100 rounded-xl">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Overtime Pay</span>
              <span className="text-sm font-bold text-green-600 mt-1 block font-mono">+₹{latestSalary.overtimePay.toLocaleString('en-IN')}</span>
            </div>
            <div className="p-3 bg-slate-50 border border-slate-100 rounded-xl">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Shift Bonuses</span>
              <span className="text-sm font-bold text-green-600 mt-1 block font-mono">+₹{latestSalary.bonus.toLocaleString('en-IN')}</span>
            </div>
            <div className="p-3 bg-slate-50 border border-slate-100 rounded-xl">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Deductions</span>
              <span className="text-sm font-bold text-rose-600 mt-1 block font-mono">-₹{latestSalary.deductions.toLocaleString('en-IN')}</span>
            </div>
            <div className="p-3 bg-slate-50 border border-slate-100 rounded-xl">
                ...
                {salaries.map((rec) => {
                  ...
                  return (
                    <tr key={rec.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3.5 font-mono">₹{rec.monthlySalary.toLocaleString('en-IN')}</td>
                      <td className="px-4 py-3.5 font-mono text-green-600">+₹{additions.toLocaleString('en-IN')}</td>
                      <td className="px-4 py-3.5 font-mono text-rose-600">-₹{rec.deductions.toLocaleString('en-IN')}</td>
                      <td className="px-4 py-3.5 font-bold font-mono text-slate-800">₹{rec.netPay.toLocaleString('en-IN')}</td>
